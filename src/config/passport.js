...
const initializePassport = () => {
	...
	const cookieExtractor = (req) => {
		/* podemos tomar desde las cookies */
		console.log("coockies que llegan", req.headers.authorization);
		/* o desde lo que nos envian desde el front */
	...
	passport.use(
		...
		new JWTStrategy(
			...
			async (jwt_payload, done) => {
				//jwt_payload contiene la info del token entendida por jwt
				try {
					//si existe la info del token se envia si no error
					console.log("este es el jwt_payload", jwt_payload);
			...
			async (username, password, done) => {
				try {
					...
					if (validatePassword(password, user.password)) {
						...
						/* Actualizamos la ultima conexion de el usuario */
